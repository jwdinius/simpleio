...
class session : public std::enable_shared_from_this<session> {
 ...
 public:
  ...
  void on_read(beast::error_code ec, std::size_t bytes_transferred) {
    ...
    // Send the response using a C++14 lambda
    auto self = shared_from_this();
    auto send = [self](auto&& msg) {
      // The lifetime of the message has to extend
      // for the duration of the async operation so
      // we use a shared_ptr to manage it.
      auto sp = std::make_shared<std::decay_t<decltype(msg)>>(
          std::forward<decltype(msg)>(msg));

      // Store a type-erased version of the shared pointer
      // in the class to keep it alive.
      self->res_ = sp;
    ...
    // Send the response
    handle_request(*doc_root_, std::move(req_), send);
  }
...
class listener : public std::enable_shared_from_this<listener> {
 ...
 public:
  listener(std::shared_ptr<net::io_context> ioc, tcp::endpoint endpoint,
           std::shared_ptr<std::string const> const& doc_root)
      : ioc_(std::move(ioc)), acceptor_(*ioc_), doc_root_(doc_root) {
    beast::error_code ec;
...
int main(int argc, char* argv[]) {
  ...
  auto ioc = std::make_shared<net::io_context>(threads);
